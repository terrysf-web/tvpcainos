...
function CreateServiceModal({ songs, onClose, onCreate, addSong }) {
  ...
  const handleCreate = async () => {
    // ── 성찬주일팀: PDF 1개 = 항목 하나. 파트별 곡 이름은 목차(songOutline)로 저장 ──
    if (CUSTOM_BRAND) {
...
function EditServiceModal({ svc, songs, addSong, onClose, onSave, onPracticeUrlSaved }) {
  ...
  const [saving,            setSaving]            = useState(false);
  // 성찬주일팀: PDF 1개 + 파트별 목차(songOutline) 편집. partNames = { part: [names] }
  ...
  const [partNames, setPartNames] = useState(() => {
    ...
    if (Array.isArray(svc.songOutline) && svc.songOutline.length) {
      svc.songOutline.forEach(g => { if (map[g.part]) g.names.forEach(n => map[g.part].push(n)); });
    } else {
      // 레거시(곡마다 항목이던 예배) → 곡 제목을 파트별로 모음
      (svc.songIds || []).forEach((id, i) => {
        const raw = (svc.songPartIds || [])[i];
        const part = SONG_SECTIONS.includes(raw) ? raw : DEFAULT_SECTION;
        if (map[part] && songMap[id]?.title) map[part].push(songMap[id].title);
      });
    }
    SONG_SECTIONS.forEach(p => { if (!map[p].length) map[p].push(""); });
    return map;
  });
  ...
  // ── 성찬주일팀: PDF 1개 항목 유지 + 파트별 목차(songOutline) + PDF 교체 저장 ──
  ...
  const SvcCard = ({ svc, past, first, justPast: jp }) => {
    ...
    return (
      <div className="wFadeIn"
        ...
        {CUSTOM_BRAND && Array.isArray(svc.songOutline) && svc.songOutline.length ? (
          /* 성찬주일팀: 파트별 목차 (경배와찬양/입례/파송) */
          <div style={{ marginBottom:8 }}>
            {svc.songOutline.map(g => (
              <div key={g.part} style={{ display:"flex", gap:8, marginBottom:4, alignItems:"baseline" }}>
                <span style={{ fontSize:11, fontWeight:800, color:SECTION_COLORS[g.part] || C.acc,
                  flexShrink:0, minWidth:62 }}>{g.part}</span>
                <span style={{ fontSize:12.5, color:C.txt, lineHeight:1.5 }}>{g.names.join(" · ")}</span>
              </div>
            ))}
          </div>
        ) : (
          ...
          return (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span style={{ fontSize:12, color:C.dim }}>{
                CUSTOM_BRAND && Array.isArray(svc.songOutline)
                  ? `${svc.songOutline.reduce((n, g) => n + g.names.length, 0)}곡`
...
function WorshipRecordingsModal({ songId, songTitle, user, svc, onClose }) {
  ...
  const del = async (rec) => { setConfirmDel(rec); };
  ...
  const saveAllLinks = async () => {
    ...
    setSaving(true); setSaveProgress("저장 중...");
    try {
      const partsData = {};
  ...
  const accessibleParts = canSeeAll ? PARTS.map(p => p.id) : ["전체", ...myParts].filter(Boolean);
  const accessibleRecs = recs.filter(r => {
    if (canSeeAll) return true;
    return r.part === "전체" || myParts.includes(r.part);
  });
  const filteredRecs = partFilter === "전체" ? accessibleRecs : accessibleRecs.filter(r => r.part === partFilter);
    ...
    <Modal title={`예배 녹음 — ${songTitle}`} onClose={onClose} noBackdrop>
      ...
      {!canSeeAll && myParts.length > 0 && (
        <div style={{ fontSize:11, color:C.dim, marginBottom:8,
          ...
          display:"flex", alignItems:"center", gap:5 }}>
          <span>🎵</span>
          <span>전체 믹스{myParts.map(p => <> · <strong key={p} style={{ color:C.acc }}>{p}</strong></>)}{" "}파트 녹음을 들을 수 있습니다</span>
        </div>
      )}
      ...
      <div style={{ maxHeight:"52vh", overflowY:"auto" }}>
        {filteredRecs.length === 0 ? (
          ...
          const embedSrc  = rec.driveId ? `https://drive.google.com/file/d/${rec.driveId}/preview` : null;
          return (
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  {!isEditing ? (<>
                    ...
                    <div style={{ fontSize:11, color:C.dim }}>
                      {rec.uploaderName} · {fmtDate(rec.createdAt)}{rec.driveId ? " · Google Drive" : ""}
                    </div>
                  </>) : (
                    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                      ...
                      <div style={{ display:"flex", gap:6 }}>
                        ...
                        <button onClick={() => setEditingId(null)} style={{
                          ...
                          color:C.dim, fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>취소</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {!showAdd ? (
            ...
            <div style={{ background:C.card, borderRadius:10, padding:12, border:`1px solid ${C.bdr}` }}>
                ...
                try {
                  await openDrivePicker((docs) => {
                    ...
                    docs.forEach((doc, idx) => {
                      if (idx < PARTS.length) newLinks[PARTS[idx].id] = `https://drive.google.com/file/d/${doc.id}/view`;
                    });
                    setPartLinks(prev => ({ ...prev, ...newLinks }));
                  });
              ...
              <div style={{ background:`${C.acc}10`, borderRadius:8, padding:10,
                ...
                <div style={{ fontSize:11, fontWeight:700, color:C.acc, marginBottom:4 }}>⚡ 한꺼번에 붙여넣기</div>
              ...
              <input placeholder="제목 (선택) — 예: 6월 7일 주일예배"
              ...
              {PARTS.map(p => {
                ...
                return (
                  <div key={p.id} style={{ marginBottom:7 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:C.dim, marginBottom:3 }}>{p.emoji} {p.label}</div>
                    <input placeholder="https://drive.google.com/file/d/..."
                      value={val} onChange={e => setPartLinks(prev => ({ ...prev, [p.id]: e.target.value }))}
                      style={{
...
function ServiceDetailScreen({ user, services, songs, annotations, teamAnnotations, userMap, notifs, nav, selectedSvcId, onUpdateService, addSong }) {
  ...
  // 성찬주일팀: 상세 화면엔 PDF 1개만 보이면 되므로 섹션(파트) 표시를 항상 끔.
  // 파트별 곡 이름은 예배 카드의 목차(songOutline)로만 노출된다.
  ...
  const setClosingSong = async (songId) => {
    ...
    // 원곡은 그대로 두고, 기존 Closing 복제만 제거한 뒤 이 곡을 끝에 Closing 으로 복제 추가 (중복)
...
function HomeSplashScreen({ user, onEnterLite }) {
  ...
  return (
    <>
      ...
      {/* Schedule cards — 그 외 팀: 파트별(보컬/밴드) 좌우 카드 */}
      {!CUSTOM_BRAND && <>
        <ScheduleCard
          title="보컬" icon="🎤"
          events={vocalEvents}
...
function LiteImageViewer({ song, onHome }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"#000" }}>
      <div style={{ width:"100%", height:"100%", overflow:"auto", WebkitOverflowScrolling:"touch" }}>
        <img
          src={song.imageUrl}
          alt={song.title}
          style={{ width:"100%", display:"block", touchAction:"pan-x pan-y pinch-zoom" }}
        />
      </div>
