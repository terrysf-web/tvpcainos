...
const eraserFrac = (w) => (w >= 8 ? 0.14 : w >= 5 ? 0.09 : 0.05);
...
function PDFViewerScreen({ user, songs, services, annotations, teamAnnotations, onAddAnnotation, onDeleteAnnotation, nav, selectedSongId, selectedSvcId, selectedSvcSongIdx, backTo, pdfjsReady, sharedGeminiKey, songCues, sendCue, deleteCue, editCue, reorderCue, userRoleMap, moveCueLabel, resizeCueLabel, sheetLinkEnabled, sheetSyncTrigger }) {
  ...
  const [drawWidth, setDrawWidth] = useState(5); // S/M/L = 3/5/8 (기본 M)
  const [eraserWidth, setEraserWidth] = useState(3); // 지우개 폭 (펜과 별개) — 항상 최소(S)로 시작, 저장 안 함
  ...
  const [pickBass,       setPickBass]       = useState(null);  // 선택한 베이스 (슬래시 코드, 반음 인덱스)
  ...
  const togglePointer = () => {
    const turningOn = !pointerOn;
    setPointerOn(turningOn);
    pointerStrokesRef.current = [];
    pointerLiveRef.current = null;
    [pointerCanvas1Ref, pointerCanvas2Ref].forEach(r => { if (r.current) drawPointerStrokes(r.current, [], null); });
    if (turningOn) {
  ...
  // 팀원: svc.teamPointer 변경 시 캔버스 렌더링
  // 포인터를 직접 켠 본인(pointerOn) 제외 + 어드민·FOH 제외(그 외 모든 파트가 포인터를 봄)
  useEffect(() => {
    if (pointerOn) return;
    if (user?.role === "admin" || isFoh(user)) return;
  ...
  const closePicker = () => { setChordInput(null); setPickRoot(null); setPickQual(""); setPickBass(null); };
  const chordPicker = (side) => {
    ...
    const assembled = pickRoot != null ? roots[pickRoot] + pickQual + (pickBass != null ? "/" + roots[pickBass] : "") : "";
      ...
      <div style={{ position:"absolute", left:`${leftPct}%`, top:`${chordInput.y * 100}%`,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, marginBottom:9 }}>
          <button onClick={() => setPickBass(null)} style={{ padding:"5px 0", borderRadius:5, fontSize:11,
            fontWeight:800, fontFamily:"monospace", cursor:"pointer",
            border:`1px solid ${pickBass==null?C.grn:C.bdr}`, background: pickBass==null?`${C.grn}18`:"#fff",
            color: pickBass==null?C.grn:C.dim }}>없음</button>
          {roots.map((r, i) => (
            <button key={i} onClick={() => setPickBass(i)} style={{ padding:"5px 0", borderRadius:5, fontSize:11,
              fontWeight:800, fontFamily:"monospace", cursor:"pointer",
              border:`1px solid ${pickBass===i?C.grn:C.bdr}`, background: pickBass===i?C.grn:"#f4f4f7",
              color: pickBass===i?"#fff":C.txt }}>{r}</button>
          ))}
        </div>
        <div style={{ display:"flex", gap:6 }}>
          ...
          <button onClick={() => { if (pickRoot==null) return; const ci = chordInput; storeChordAt(ci, assembled); closePicker(); }}
  ...
  // 포인터 켜진 동안 소스가 악보를 넘기면 teamPointer.songId만 갱신 (악보 동기화 없음 —
  // 같은 곡을 보고 있는 팀원에게만 포인터가 보임). 듀얼은 dualLeftSongId(dualIdx 연동)도 감시.
  useEffect(() => {
    if (!pointerOn || (!leader && user?.role !== "admin")) return;
    const currentSongId = (dual && dualLeftSongId) ? dualLeftSongId : selectedSongId;
    if (!currentSongId || !svc?.id) return;
  ...
  const handleChordPointerMove = (e, side) => {
    if (longPressOrigin.current) {
      ...
      // cancel long-press if pointer moved > 5px (non-swipe) — 드래그 중 실수 삭제 방지
      if (dx * dx + dy * dy > 25) {
        clearTimeout(longPressTimer.current);
        setDeletingChord(null);
        longPressOrigin.current = null;
  ...
  const confirmText = useCallback(async () => {
    ...
    const textStroke = {
      ...
      color: activeColor, size: ({ 3: 8, 5: 15, 8: 28 })[drawWidth] || 15,
    ...
    <div style={{ position:"fixed", inset:0, background:C.bg, display:"flex",
              ...
              return (
                <button key={name}
                  onClick={() => setActiveGroup(g => {
                    const next = g === name ? null : name;
                    // 필기 그룹을 닫거나 다른 그룹으로 전환 시 필기 관련 툴 모두 끔
                    // → 화면 넘기기 잠금 해제 (필기 툴바만 사라지고 잠긴 채 남는 문제 수정)
                    if (g === "필기" && next !== "필기") {
                      setDrawMode(false);
              ...
              <div className="pdf-toolbar-btns" style={{ display:"flex", gap:3, alignItems:"center", flexShrink:1, minWidth:0,
                ...
                {(leader || user?.role === "admin") && !isLibraryMode && (
                  <button onClick={togglePointer} style={{
                    position:"relative", flexShrink:0, height:28,
                    padding: tbNarrow ? "0 6px" : "0 8px",
                    background: pointerOn ? "#fff" : "rgba(255,255,255,0.12)",
                    border:`1px solid ${pointerOn ? "#fff" : "rgba(255,255,255,0.3)"}`,
                    borderRadius:7, cursor:"pointer",
                    display:"flex", alignItems:"center", gap:3,
                    color: pointerOn ? "#c0392b" : "#fff",
                    fontWeight: pointerOn ? 800 : 700,
                    fontSize: tbNarrow ? 10 : 11, fontFamily:"inherit",
                  }}>
      ...
      {isLiteMode && (() => {
        ...
        return (<>
          ...
          {/* 펜 버튼 (드로잉 모드 토글) */}
          {liteBtn(() => {
            if (!drawMode) { setDrawMode(true); setDrawTool("pen"); setDrawWidth(5); setDrawColor("#1a6fe8"); }
          ...
          {/* 지우개 버튼 — 필기 모드 ON일 때만 */}
          {drawMode && liteBtn(() => setDrawTool(t => { const nt = t === "eraser" ? "pen" : "eraser"; if (nt === "eraser") setEraserWidth(3); return nt; }), (<>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
          ...
          {(() => {
            ...
            const utilBar = (mLeft) => (
              <div style={{ marginLeft: mLeft ? "auto" : 0, display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
                ...
                {chordMoveMode && (chordData.length > 0 || chordData2.length > 0) && <span style={{ fontSize:9, color:C.dim, whiteSpace:"nowrap" }}>더블탭: 복사 · 길게 꾹: 삭제</span>}
            ...
            <div style={{ width:"100%" }}>
                    ...
                    return (
                      <div key={cue.id} style={{ padding:"8px 10px", borderRadius:8,
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
                          <span style={{ fontSize:11, fontWeight:800, color:"#e65c00", display:"flex", alignItems:"center", gap:5 }}>
                            {cue.userPart || cue.userName}
                            {cue.section && cue.section.trim() && (
                              <span style={{ fontSize:10, fontWeight:700, color:"#fff", background:"#ff6f00",
              ...
              {(() => {
                const usedSections = [...new Set(
                  (songCues?.[cueSongId] || []).map(c => c.section).filter(s => s && s.trim())
                )];
                return (
